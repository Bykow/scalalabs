...
object Anagrams extends App {
  ...
  def sentenceAnagrams(sentence: Sentence) : List[Sentence] = {
    ...
    def helper(print: FingerPrint) : List[Sentence] = {
      if (print.isEmpty) List(List())
      else {
        for {
          subprints <- subseqs(print)
          possibleWords <- wordAnagrams(subprints)
          newPrint <- helper(subtract(print, subprints))
        } yield possibleWords :: newPrint
      }
    }

    helper(fingerPrint(sentence))
  }
